...
def collect_all_crimes():
    ...
    while True:
        ...
        if crimes:
            print(f"Latest crime ID is {crimes[-1].crime_id} and new crime ID is {new_crimes[0].crime_id}")
            print(f"Latest crime date is {crimes[-1].date} and new crime date is {new_crimes[0].date}")
